<template>
    <div v-if="active">
      <div class="fixed top-4 right-4 bg-yellow-200 text-black p-4 rounded-lg shadow-lg z-50  contrast:bg-contrast-primary contrast:text-contrast-primary">
        {{ message }}
      </div>
    </div>
  </template>
  ...
  const message = ref('Klicken Sie auf die Karte, um einen Kreis zu zeichnen')
  ...
  const initializeDraw = () => {
    ...
    draw.value.on('drawstart', () => {
      message.value = 'Klicken Sie und ziehen Sie, um einen Kreis zu zeichnen'
    })
    ...
    draw.value.on('drawend', (event) => {
      ...
      let formattedRadius
      if (radius > 1000) {
        formattedRadius = `${Math.round((radius / 1000) * 100) / 100} km`
      } else {
        formattedRadius = `${Math.round(radius * 100) / 100} m`
      }
      
      message.value = `Gemessener Radius: ${formattedRadius}`
    })
  ...
  const cleanup = () => {
    ...
    message.value = 'Klicken Sie auf die Karte, um einen Kreis zu zeichnen'
  }
